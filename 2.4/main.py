...
while "anime" == "anime":
    ...
    if command == "getip":
        ...
        cmds.getlocalip()
    elif "getsiteip" in command:
        cmds.getdomainip(command[10:].replace('http://', '').replace('https://', ''))
    elif "sitestatus" in command:
        cmds.sitestatus(command[11:].replace('http://', '').replace('https://', ''))
    elif command == "getmac":
        cmds.getmac()
    elif command == "getmacnum":
        cmds.getmacnum()
    elif "rangeports" in command:
        na4alo = int(input("Введите начальный порт\n"))
        predel = int(input("Введите последний порт\n"))
        ip = command[11:].replace('http://', '').replace('https://', '')
        cmds.scanports(ip, na4alo, predel)
        print("Сканирование завершено")
    elif "scanports" in command:
        ip = command[10:].replace('http://', '').replace('https://', '')
        cmds.scanports(ip, 1, 65535)
        print("Сканирование завершено")
    ...
    elif "blake2" in command:
        cmds.blake2(command[8:])
    elif "getdns" in command:
        cmds.getdns(command[7:].replace('http://', '').replace('https://', ''))
    elif "getssl" in command:
        cmds.getssl(command[7:].replace('http://', '').replace('https://', ''))
    elif "saveqr" in command:
        cmds.saveqr(command[7:])
    elif "qr" in command:
        cmds.qr(command[3:])
    else:
