...
def gethelp():
    ...
    getsiteip [ДОМЕН] - получение IP сайта
    sitestatus [ДОМЕН] - проверка статуса сайта
    getmac - получить MAC-адрес
    getmacnum - получить MAC-адрес в виде числа
    scanports [IP/ДОМЕН] - проверить весь диапазон портов
    rangeports [IP/ДОМЕН] - проверить заданный диапазон портов
    ...
    blake2 [ТЕКСТ] - хеширование в blake2b
    getdns [IP/ДОМЕН] - перечисление DNS
    getssl [IP/ДОМЕН] - проверка SSL
    qr [ТЕКСТ] - создать и отобразить QR-код
    saveqr [ТЕКСТ] - создать и сохранить QR-код
    """)
...
def checkpass():
    import passwordmaster
    passwordmaster.checker(input("Введите пароль\n"))
...
def ping(site):
    import subprocess
    print(subprocess.run(['ping', site], capture_output=True).stdout.decode('cp866', errors='replace'))

def tracert(site):
    import subprocess
    print(subprocess.run(['tracert', site], capture_output=True, encoding='cp866', errors='replace').stdout)
...
def qr(text):
    import segno
    segno.make_qr(text).show()

def saveqr(text):
    import segno
    segno.make_qr(text).save("qr.png", scale=20, border=1)
